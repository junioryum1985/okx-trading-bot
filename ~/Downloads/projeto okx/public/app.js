  ...
  cfg.strategies.forEach(s => {
    ...
    div.innerHTML = `
      <div class="strat-header">
        ...
        <span class="strat-badge ${s.direction}" id="badge-${s.id}">${s.direction}</span>
        <span class="strat-status" id="status-${s.id}">⏳ aguardando</span>
      </div>
      <div class="strat-details">
        ${s.timeframe} | ${s.leverage}x | TP ${s.tp_pct}% | SL ${s.sl_pct}%
      </div>
      <div class="strat-pos" id="pos-${s.id}">
  ...
  try {
    ...
    switch (data.type) {
      ...
      case 'signal':
        ...
        document.getElementById(`status-${data.strategy}`).className = `strat-status ${data.signal ? 'signal-on' : ''}`;
        break;
      case 'position':
        document.getElementById(`status-${data.strategy}`).textContent = '📌 EM POSIÇÃO';
        document.getElementById(`status-${data.strategy}`).className = 'strat-status in-position';
      ...
      case 'trade':
        trades.push(data);
        updateTradeChart();
        addLog(`[${data.strategy}] Trade ${data.side.toUpperCase()} | Entry: ${formatUSD(data.entryPx)} | TP: ${formatUSD(data.tpPx)} | SL: ${formatUSD(data.slPx)}`);
        break;
    }
  } catch (e) { /* ignore */ }
};
...
function updateTradeChart() {
  ...
  if (!tradeChart) {
    ...
    tradeChart = new Chart(ctx, {
      type: 'scatter',
      data: {
        datasets: Object.keys(colors).map(name => ({
          label: name, data: [], backgroundColor: colors[name], pointRadius: 6
        }))
      },
      options: {
  ...
  trades.forEach(t => {
    ...
    if (dsIdx >= 0) {
      const exists = tradeChart.data.datasets[dsIdx].data.some(d => d.x === t.ts);
      if (!exists) tradeChart.data.datasets[dsIdx].data.push({ x: t.ts, y: t.entryPx });
    }
  });
